...
Сервис для сокращения ссылок с JWT-аутентификацией на асимметричных ключах, кэшированием, генерацией QR-кодов и фоновыми задачами через Celery.

- Чистая архитектура (Service → Repository)
- JWT access (15 минут) + refresh (7 дней) с **RSA подписью**
- **Argon2** хэширование паролей
- **SHA-256** хэширование refresh-токенов в БД
- Изолированное кэширование в Redis (db=0 для кэша и QR-кодов, db=1 для SlowAPI, db=2 для Celery)
...
REST API с JWT авторизацией, кэшем и пагинацией. С фронтендом, сгенерированным с ИИ.
